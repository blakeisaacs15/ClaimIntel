...
function exhibitLabel(i: number) { return `Exhibit ${EXHIBIT_LABELS[i] ?? i + 1}`; }
...
export default function PreAuthModal({ input, onClose }: PreAuthModalProps) {
  ...
  const [lastGeneratedProviderId, setLastGeneratedProviderId] = useState("");
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const generate = async (providerOverride?: { full_name: string; npi: string | null }) => {
    setLoading(true);
    setError(null);
    setLetter("");
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch("/api/generate-preauth", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session?.access_token ?? ""}`,
        },
        body: JSON.stringify({ ...input, providerOverride }),
      });
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (!cancelled) {
        const override = defaultProvider
          ? { full_name: (defaultProvider as Provider).full_name, npi: (defaultProvider as Provider).npi }
          : undefined;
        await generate(override);
      }
    };
    init().catch(() => {
      if (!cancelled) { setError("Failed to generate letter. Please try again."); setLoading(false); }
    });
  ...
  const handleRegenerate = async () => {
    ...
    const override = prov ? { full_name: prov.full_name, npi: prov.npi } : undefined;
    await generate(override);
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      for (const { file, index } of exhibitOrder) {
        ...
        doc.addImage(imgData, fmt, (pageW - props.width * ratio) / 2, 20, props.width * ratio, props.height * ratio);
      }

      const pdfAttachments = attachments.filter(f => f.type === "application/pdf");
      const codeSlug = input.codes[0] ?? "preauth";
      const payerSlug = input.payer.toLowerCase().replace(/\s+/g, "-");
      const fileName = `preauth-${codeSlug}-${payerSlug}.pdf`;
      ...
      for (const { file, index } of exhibitOrder) {
        ...
        cover.drawText(exhibitLabel(index).toUpperCase(), { x: 36, y: 766, size: 13, font, color: rgb(0.2, 0.2, 0.2) });
        cover.drawText(file.name, { x: 36, y: 746, size: 10, font: fontNormal, color: rgb(0.45, 0.45, 0.45) });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100 flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-gray-900">Pre-Authorization Request</h2>
            <p className="text-xs text-gray-400 mt-0.5">
              {input.codes.join(", ")} &middot; {input.payer} &middot; Age {input.patientAge}
              {input.toothNumber ? ` · Tooth #${input.toothNumber}` : ""}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 min-h-0">
          {loading && (
            <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-400">
              ...
              <p className="text-sm">Generating pre-authorization letter...</p>
            </div>
          )}
          ...
          {error && !loading && (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <button onClick={handleRegenerate} className="text-sm text-teal-600 underline">Retry</button>
            </div>
          )}
            ...
            <>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                  ...
                  <span className="ml-2 font-normal normal-case text-gray-400">X-rays and clinical records — appended as exhibits in the PDF</span>
                </p>
                <div
                ...
                >
                  ...
                  />
                  <p className="text-sm text-gray-500">Drop files here or <span className="text-teal-600 font-medium">browse</span></p>
