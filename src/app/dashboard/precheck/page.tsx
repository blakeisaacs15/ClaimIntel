...
export default function PrecheckPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validCodes.length) {
      setError("Enter at least one valid procedure code (e.g. D2740).");
      return;
    }
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              ...
              {historyLoading ? (
                ...
                <div className="divide-y divide-gray-100">
                  {history.map((entry) => {
                    const codes = (entry.results ?? []).map((r) => r.code).join(", ");
                    ...
                    return (
                      <div key={entry.id} className="px-6 py-3 flex items-center gap-4 text-sm">
                        ...
                        <span className="font-mono text-xs text-gray-700 flex-shrink-0 w-32 truncate">
                          {codes}
                        </span>
