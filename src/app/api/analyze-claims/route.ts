...
function parseCSV(csvData: string) {
  ...
  return lines.slice(1).map((line, i) => {
    ...
    return {
      ...
      procedure: row['procedure_code'] || row['procedure'] || row['cdt_code'] || row['proc_code'] || row['service_code'] || '—',
      amount: parseFloat((row['billed_amount'] || row['amount'] || row['billed'] || row['charge'] || '0').replace(/[$,]/g, '')) || 0,
      reason: row['denial_reason'] || row['reason'] || row['denial_description'] || row['adjustment_reason'] || row['remark'] || row['description'] || '—',
